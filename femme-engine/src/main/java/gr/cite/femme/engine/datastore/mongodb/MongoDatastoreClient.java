...
import gr.cite.femme.engine.datastore.mongodb.codecs.MetadatumCodecProvider;
...
//	private static final String DATABASE_HOST = "es-devel1.local.cite.gr:27017";
	...
	public MongoDatastoreClient(String host, int port, String name) {
		...
		this.collections = this.database.getCollection(MongoDatastoreClient.COLLECTIONS_COLLECTION_NAME, Collection.class).withCodecRegistry(codecRegistry);
		this.dataElements = this.database.getCollection(MongoDatastoreClient.DATA_ELEMENTS_COLLECTION_NAME, DataElement.class).withCodecRegistry(codecRegistry);
